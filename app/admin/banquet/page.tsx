  ...
  const addCategory = async () => {
    ...
    try {
      const res = await fetch('/api/menu', {
        ...
        body: JSON.stringify({ type: 'category', id: 'banquet-' + slugify(newCat.name), name: newCat.name.trim(), emoji: newCat.emoji }),
      });
      if (res.ok) {
  ...
  return (
    <div className="admin-page-container" style={{ maxWidth: 860 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28, flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 800, letterSpacing: '-.02em', marginBottom: 4 }}>🥂 Банкетне меню</h1>
          <p style={{ fontSize: 13, color: 'var(--text-muted)' }}>{categories.length} категорій</p>
        </div>
